            ...
            for (int itr = 0; itr < lineChar.Length; itr++)
                ...
                if (operators.Success)
                    ...
                    if(!keywordList.Contains(finalArrayc[x].ToString())) // if it is not a keyword
                    {
                            ...
                            Regex reg2 = new Regex(@"^(String|char)\s([A-Za-z|_][A-Za-z|0-9]{0,10})\s(=)\s[']\s([AZa-z|_][A-Za-z|0-9]{0,30})\s[']\s(;)$"); // line of type String alpha = ' Hello ';
                            ...
                            else if (category2.Success)
                            {
                                ...
                                if (!(finalArrayc[x].ToString().Equals("'") && finalArrayc[x + 1].ToString().Equals("'")))
                                {
                                    SymbolTable[row, 1] =
                                   row.ToString(); // index
                                    SymbolTable[row, 2] =
